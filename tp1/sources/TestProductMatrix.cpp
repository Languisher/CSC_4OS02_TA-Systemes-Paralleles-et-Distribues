...
prod_algo parseAlgo(const std::string& algoName) {
  if (algoName == "naive") return naive;
  if (algoName == "ijk") return ijk;
  if (algoName == "jik") return jik;
  if (algoName == "ikj") return ikj;
  if (algoName == "kij") return kij;
  if (algoName == "jki") return jki;
  if (algoName == "kji") return kji;
  if (algoName == "block") return block;
  if (algoName == "parallel_naive") return parallel_naive;
  if (algoName == "parallel_block1") return parallel_block1;
  if (algoName == "parallel_block2") return parallel_block2;
  std::cerr << "Unknown algo '" << algoName << "', fallback to naive.\n";
  return naive;
}
...
int main(int nargs, char *vargs[])
{
  ...
  if (nargs > 1)
    dim = atoi(vargs[1]);
  if (nargs > 2)
    algo = parseAlgo(vargs[2]);
  if (nargs > 3)
    blockSize = atoi(vargs[3]);
  if (nargs > 4)
    nbThreads = atoi(vargs[4]);
  ...
  if (isPassed)
    {
      ...
      std::cout << "Configuration: dim=" << dim
                << ", algo=" << (nargs > 2 ? vargs[2] : "naive")
                << ", blockSize=" << blockSize
                << ", threads=" << (nbThreads > 0 ? nbThreads : 0) << "\n";
      std::cout << "Temps CPU produit matrice-matrice : " << elapsed_seconds.count() << " secondes\n";
