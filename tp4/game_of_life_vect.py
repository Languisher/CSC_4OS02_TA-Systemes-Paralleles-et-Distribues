...
class Grille:
    ...
    def compute_next_iteration(self):
        ...
        """
        # Version vectorisée (grille torique) : somme des 8 voisins via décalages circulaires.
        cells = self.cells
        voisins = (
            np.roll(np.roll(cells, 1, axis=0), 1, axis=1)   # haut-gauche
            + np.roll(cells, 1, axis=0)                     # haut
            + np.roll(np.roll(cells, 1, axis=0), -1, axis=1)  # haut-droite
            + np.roll(cells, 1, axis=1)                     # gauche
            + np.roll(cells, -1, axis=1)                    # droite
            + np.roll(np.roll(cells, -1, axis=0), 1, axis=1)  # bas-gauche
            + np.roll(cells, -1, axis=0)                    # bas
            + np.roll(np.roll(cells, -1, axis=0), -1, axis=1)  # bas-droite
        )

        next_cells = np.where(
            (voisins == 3) | ((cells == 1) & (voisins == 2)),
            1,
            0,
        ).astype(np.uint8)

        diff_cells = np.flatnonzero(next_cells != self.cells).tolist()
